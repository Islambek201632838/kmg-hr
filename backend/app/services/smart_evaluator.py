...
Если smart_index < 0.7 — предложи improved_goal: переформулировку которая:
1. Тип цели = "impact" (влияние на бизнес), НЕ activity
2. Привязка = "strategic" или "functional" — к конкретному KPI или ВНД из контекста
3. Содержит количественную метрику и конкретный дедлайн
4. smart_index переформулировки должен быть >= 0.85
...
Ответ СТРОГО в JSON:
{
  ...
  "smart_index": 0.0,
  "goal_type": "output",
  "strategic_alignment": {"level": "functional", "source": "KPI подразделения"},
  "recommendations": ["S: ...", "M: ..."],
  "improved_goal": null
}"""
...
def build_user_prompt(
    ...
    if rag_chunks:
        parts.append("\nРелевантные фрагменты ВНД (для оценки стратегической связки):")
        for ch in rag_chunks[:3]:
            parts.append(f"\n[{ch.get('doc_title', '')}] (score: {ch.get('score', 0):.2f})")
            parts.append(ch.get("text", "")[:300])
...
async def evaluate_goal(
    ...
    user_prompt = build_user_prompt(
        goal_text, position, department, manager_goals, kpis, historical_goals, rag_chunks
    )
