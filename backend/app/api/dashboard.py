...
async def _get_base_data(remote, local):
    """Shared data fetching for dashboard endpoints."""
    dept_result = await remote.execute(select(Department).where(Department.is_active == True))
    departments = dept_result.scalars().all()

    emp_result = await remote.execute(select(Employee.id, Employee.department_id))
    emp_to_dept = {r[0]: r[1] for r in emp_result.all()}

    evals_result = await local.execute(select(AiEvaluation))
...
async def department_quality(
    ...
    # Goals count per dept/quarter
    goals_q = (
        select(Goal.department_id, Goal.quarter, func.count(Goal.goal_id).label("cnt"))
        .where(Goal.year == year)
        .group_by(Goal.department_id, Goal.quarter)
    )
    goals_result = await remote.execute(goals_q)
    dept_total = defaultdict(int)
    for r in goals_result.all():
        dept_total[r.department_id] += r.cnt

    # Build eval → quarter mapping via goal_quarter_map
    eval_quarter_map: dict[int, str] = {}
    for ev in evals:
    ...
    for dept in departments:
        ...
        if d_evals:
            ...
            criterion_sums = {"specific": 0, "measurable": 0, "achievable": 0, "relevant": 0, "time_bound": 0}
            for e in d_evals:
                if e.scores_json:
                    for k in criterion_sums:
                        criterion_sums[k] += e.scores_json.get(k, 0)
            if any(criterion_sums.values()):
                weakest = min(criterion_sums, key=criterion_sums.get)

        strategic_ratio = strategic_count / len(d_evals) if d_evals else 0.0

        for q_name in ["Q1", "Q2", "Q3", "Q4"]:
            q_evals = [e for e in d_evals if eval_quarter_map.get(e.id) == q_name]
    ...
    top_issues = []
    if evals:
        activity_pct = sum(1 for e in evals if e.goal_type == "activity") * 100 // len(evals)
        if activity_pct > 0:
            top_issues.append(f"{activity_pct}% целей сформулированы как активности")
        no_strat_pct = sum(1 for e in evals if e.alignment_level != "strategic") * 100 // len(evals)
        if no_strat_pct > 0:
...
async def department_maturity(
    ...
    d_evals = dept_evals.get(department_id, [])
    ...
    # SMART quality
    smart_quality = sum(e.overall_index for e in d_evals) / len(d_evals)
    ...
    for e in d_evals:
        ...
        if t in type_counts:
            type_counts[t] += 1
    type_dist = {k: round(v / len(d_evals), 2) for k, v in type_counts.items()}
    ...
    # Maturity index: weighted composite
    maturity = round(
        0.40 * smart_quality +
    ...
    if type_dist.get("activity", 0) > 0.2:
        pct = int(type_dist["activity"] * 100)
        recs.append(f"{pct}% целей сформулированы как активности. Рекомендуется переформулировать в результат (output/impact)")
    if strategic_ratio < 0.5:
        recs.append(f"Только {int(strategic_ratio * 100)}% целей связаны со стратегией. Рекомендуется усилить стратегическую привязку")
    if criterion_avgs.get("measurable", 1) < 0.6:
        recs.append("Низкий показатель измеримости. Рекомендуется добавить количественные метрики к целям")
    if criterion_avgs.get("time_bound", 1) < 0.6:
        recs.append("Низкий показатель ограниченности во времени. Рекомендуется указывать конкретные дедлайны")
...
async def quarterly_trend(
    ...
    """
    Возвращает динамику качества целеполагания по кварталам для подразделения.
    Используется для линейного тренд-графика на дашборде.
    """
    departments, emp_to_dept, evals, dept_evals = await _get_base_data(remote, local)
    goal_quarter_map = await _get_goal_quarter_map(remote, year)
    ...
    d_evals = dept_evals.get(department_id, [])
    eval_quarter_map = {
        ev.id: goal_quarter_map[ev.goal_id]
        for ev in d_evals
        if ev.goal_id and ev.goal_id in goal_quarter_map
    }
    ...
    for q in ["Q1", "Q2", "Q3", "Q4"]:
        q_evals = [e for e in d_evals if eval_quarter_map.get(e.id) == q]
        if q_evals:
    ...
    return {"department": dept.name, "year": year, "trend": trend}
