...
|-------------------|-------------------|
| Выбор сотрудника из выпадающего списка | SSO через **Keycloak** / Active Directory |
| Нет разграничения прав | **RBAC**: сотрудник видит свои цели, руководитель — команду, HR — дашборд |
| Все эндпоинты публичные | JWT-токены, middleware проверки ролей |

### 2. Парсинг ВНД в продакшене
| Текущее состояние | Целевое состояние |
|-------------------|-------------------|
| ВНД из БД организаторов (чистый текст) | Парсинг реальных Word (.docx) и PDF файлов |
| Один чанкер (512 токенов) | **Adaptive chunking**: учёт заголовков, таблиц, списков |
| Нет OCR | **OCR-пайплайн** для сканированных PDF (Tesseract / Azure Document Intelligence) |
| Нет версионирования | Отслеживание версий ВНД, автообновление Qdrant при изменении документа |

### 3. Локальное развёртывание LLM (GPU-кластер)
| Текущее состояние | Целевое состояние |
|-------------------|-------------------|
| Gemini Flash (Google Cloud API) | Локальная LLM на GPU-кластере (Llama 3, Mistral, Qwen) |
| ВНД отправляются в облако | **ВНД остаются внутри контура** — compliance, NDA, закон о персональных данных |
| Зависимость от внешнего API | **vLLM / TGI** на NVIDIA A100/H100 — полная автономность |
| Rate limits облака | Масштабирование под нагрузку без ограничений RPM |

### 4. Масштабирование и отказоустойчивость
| Текущее состояние | Целевое состояние |
|-------------------|-------------------|
| 1 uvicorn worker | **Gunicorn + N workers** / Kubernetes HPA |
| Нет rate limiting | **Redis + token bucket**: 10 req/sec на пользователя |
| Нет очереди | **Celery + Redis**: тяжёлые batch-оценки в фоне |
| In-memory кеш | **Redis кеш** с TTL (сотрудники, дашборд, RAG-результаты) |
| Docker Compose | **Kubernetes**: auto-scaling, health probes, rolling updates |
| 1 сервер | **3+ реплики** API за load balancer (Nginx / Traefik) |
